...
def add_extra_bytes_if_needed(short_bytes: bytes) -> Tuple[bytes, int]:
...
def bytes_to_chunks(data: bytes) -> List[UstreamChunk]:
    chunks = []
    ...
    last_chunk, length = add_extra_bytes_if_needed(last_chunk)
...
def chunks_to_bytes(chunks: List[UstreamChunk]) -> bytes:
    ...
    body = base64.b64decode(b''.join(byte_chunks))

    tail_decoded = base64.b64decode(chunks[-1].data)
